    ...
    public partial class Form1 : Form
    {
        ...
        private void setButtons_labels()
        {
            ...
            if(x == rom[0x014D])
            {
                ...
                if (ends != 16)
                    ends -= 4;
                byte[] Title2 = new byte[ends];
                for (int i = 0; i < ends; i++)
                    Title2[i] = Title[i];

                String t = System.Text.Encoding.UTF8.GetString(Title2);
                if (t.IndexOf('&') != -1)
                    t = t.Insert(t.IndexOf('&'), "&");
                ROM_Title = new System.Text.ASCIIEncoding().GetString(Title2);
                if(ROM_Title.LastIndexOf('?') != -1)
                    ROM_Title.Remove(ROM_Title.LastIndexOf('?') - 4);
                System.Text.ASCIIEncoding enc = new System.Text.ASCIIEncoding();
                Title_l.Text = "Title: " + t;
